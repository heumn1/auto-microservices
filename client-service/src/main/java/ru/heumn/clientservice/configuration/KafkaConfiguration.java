package ru.heumn.clientservice.configuration;
...
public class KafkaConfiguration {
    ...
    //you can also skip mapping if you use the same file from the same library. (identical package names)
    @Value("${spring.kafka.consumer.properties.spring.json.type.mapping}")
    String mapping;

    @Bean
    DefaultKafkaConsumerFactory configConsumer() {
    ...
    ConcurrentKafkaListenerContainerFactory<String, Object> kafkaListenerContainerFactory(
        ConsumerFactory<String, Object> consumerFactory, KafkaTemplate kafkaTemplate) {
            DefaultErrorHandler errorHandler = new DefaultErrorHandler(new DeadLetterPublishingRecoverer(kafkaTemplate), new FixedBackOff(5000, 10));
